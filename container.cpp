...
#include <string>
#include <sstream>
void findmissingLetter(std::string storeReading, std::unordered_set<std::string> storeDictionary, std::ofstream& suggestedWords);
void removeLetter(std::string breakWord, std::unordered_set<std::string> storeDictionary, std::ofstream& suggestedWords);
void replaceLetter(std::string breakWord, std::unordered_set<std::string> storeDictionary, std::ofstream& suggestedWords);
void swapLetters(std::string breakWord, std::unordered_set<std::string> storeDictionary, std::ofstream& suggestedWords);
int main(int argc, char *argv[]){

	//create a new text file to store suggestions
	std::ofstream suggestedWords("suggestions.txt");
	//read the dictionary
	std::unordered_set<std::string> storeDictionary;
	...
	if(readingFile.is_open()){
		...
		for(int l=0; l<storeReading.size(); l++){
			std::unordered_set<std::string>::iterator it=storeDictionary.find(storeReading[l]);
		...
		std::cout <<"\nTime passed to check the file in the UNORDERED_SET is: " << elapsed_seconds.count() <<" s" <<std::endl;
	}
return 0;
}
...
void findmissingLetter(std::string breakWord, std::unordered_set<std::string> storeDictionary, std::ofstream& suggestedWords){
	for(int j=0; j<=breakWord.size(); j++){
		...
		for(int n=0; n<26; n++){
			...
			std::unordered_set<std::string>::iterator it=storeDictionary.find(subStr3);
			if(it != storeDictionary.end()){
...
void removeLetter(std::string breakWord, std::unordered_set<std::string> storeDictionary, std::ofstream& suggestedWords){
	for(int j=0; j<breakWord.size(); j++){
		...
		subStr.erase(j,1);
		std::unordered_set<std::string>::iterator it=storeDictionary.find(subStr);
		if(it != storeDictionary.end()){
			suggestedWords << "Suggested word for " << breakWord <<": " <<std::endl;
...
void replaceLetter(std::string breakWord, std::unordered_set<std::string> storeDictionary, std::ofstream& suggestedWords){
	for(int j=0; j<breakWord.size(); j++){
		...
		for(int n=0; n<26; n++){
			...
			std::unordered_set<std::string>::iterator it=storeDictionary.find(subStr4);
			if(it != storeDictionary.end()){
				suggestedWords << "Suggested word for " << breakWord <<": " <<std::endl;
